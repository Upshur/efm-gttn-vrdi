...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Evolve | Oynatılan Şarkı")
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Evolve | Kuyruk Bitti!")
  .setDescription(`  **| Kuyruktaki tüm müzikler oynatıldı. Evolve kanaldan ayrılıyor.\n\nEvolve Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
